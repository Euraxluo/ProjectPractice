...
public class Consumer {
    ...
    private void testGetUser() {
        try {
            User user = new User();
//            User user1 = userProvider.GetUser("A003",user);
            ...
            System.out.println("testGetUser1");

            //链接java时需要小写
//            User user2 = userProvider.sayHi("A003","JAVANAME",200);
            //链接go时需要大写
            User user2 = userProvider.SayHi("A003","JAVANAME",200);

            System.out.println("[" + new SimpleDateFormat("HH:mm:ss").format(new Date()) + "] " +
                    " UserInfo, Id:" + user2.getId() + ", name:" + user2.getName()
                    + ", age:" + user2.getAge() + ", time:" + user2.getTime().toString());
            System.out.println("testGetUser2");
