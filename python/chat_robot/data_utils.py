...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '1'#我的电脑跑不起来GPU的,忽略AVX扩展警告

'''
定义工具类
'''

VOCAB_SIZE_THRESHOLD_CPU = 1000 #临界值:1千词向量,gpu很烂
...
# 根据输入输出的字典大小,以及GPU的信息,选择在CPU还是GPU上执行Embedding
...
def batch_flow(data, ws, batch_size, raw=False, add_end=True):
    ...
    '''
    是否返回原始对象，如果是True，假设结果是ret，那么len(ret) == len(data)*3
...
def batch_flow_bucket(data, ws, batch_size, raw=False,
    ...
    if isinstance(ws, (list, tuple)): #类型判断
        assert len(ws) == len(data), 'len(ws)必须等于len(data)'
    if isinstance(add_end, bool): #类型判断
        add_end = [add_end] * len(data)
    else:
        assert (isinstance(add_end, (list, tuple))), 'add_end不是boolean'
        assert len(add_end) == len(data), '如果add_end是list(tuple),那么add_end长度应该和输入数据的长度一样'
...
if __name__ == '__main__':
    ...
    #test_transform_sentence()
    # np.random.choice(random(10000),9999,replace=False)
    # data = [[[1],[2],[3]],[[4],[5],[6]]]
    # all_data = list(zip(*data))
