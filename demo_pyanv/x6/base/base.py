...
from base.helper import parse_size,_camelify,none_max, none_min
...
class LinkInterface(Element):
    ...
    def to_dict(self, depth=-1, **kwargs):
        """Returns a dict representation of the object."""
        out = super(LinkInterface, self).to_dict(depth=-1, **kwargs)
        out['url'] = self.url
        return out


class JavascriptLink(LinkInterface):
...
class DataLink(LinkInterface):
    ...
    """
    _template = Template(
        '{% if kwargs.get("embedded",False) or this.download or kwargs.get("download",False)  %}'
        '<script>let {{this._name}} ={{ this.to_json() }};</script>'
        '{% else %}'
        '<script src="{{this.url}}"></script>'
        '{% endif %}'
    )

    def __init__(self, name,url, download=True):
