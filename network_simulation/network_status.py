...
class NetworkStatus():
        ...
        if chain_analysis == True:
            ...
            for col_num in range(snapshot.shape[1]):
                ...
                for depth_num, block in enumerate(cur_snapshot):
                    ...
                    cur_chain_hash = hashlib.sha256(str_chain_depth.encode('utf-8')).hexdigest()
    ...
    def plot_branches(self):
        ...
        plt.ion()
        ticks = list(self.unique_chains.keys())
        ticks.sort()
        ...
        for i, tick in enumerate(ticks):
            ...
            res = list(self.unique_chains[tick])
            axarr[0][i].scatter(range(len(res)), res)
            ## other plot
            # axarr[1][i].scatter(range(len(self.unique_chains[tick])), self.current_partition.values()[:(tick+1)])
            # axarr[1][i].scatter(range(len(self.unique_chains)),list( self.current_partition.values())[:(tick+1)])
            axarr[1][i].scatter(range(len(res)), list(self.current_partition.values())[:(tick + 1)])
        fig.show()
