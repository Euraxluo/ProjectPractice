...
class Block():
    def __init__(self, initial_validator_set = [], parent=None, created_by = None, created_at = 0, nonce = ''):
        ...
        self.parent = parent
        X = str(random.randrange(10**30)) if parent is None else str(random.randrange(10**30)) + parent.hash
        self.hash = hashlib.sha256(X.encode("utf-8")) .hexdigest()
...
class Node():
    ...
    def get_branch_split_time(self, current_block, time):
        ...
        node_block_hashes = {k: v for k, v in self.chain.items() if k <= previous_node_hash_time}
...
#        current_block_hashes = {k: v for k, v in current_block_hashes.iteritems() if v != 0}
    ...
    def get_current_lockout(self, branch_time):
        ...
        branch_slots = {k:v for k,v in self.chain.items() if k >= branch_time}
  ...
  #          return self.lockouts[self.chain[branch_time]]
    ...
    def tick(self, _time):
        ...
        if self.network.round_robin[_time % len(self.network.nodes)] == self.id:
            ...
            last_block_time = max([block_time for block_time, block in self.chain.items() if block])
