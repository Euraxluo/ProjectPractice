...
import re
from Tempverb import Tempverb, TempvarbSyntaxError
...
class TempliteTest(TestCase):
    ...
    def try_render(self, text, ctx=None, result=None):
        ...
        """
        actual = Tempverb(text).render(ctx or {})
        if result:
            self.assertEqual(actual, result)

    def assertSynErr(self, msg):
        pat = "^" + re.escape(msg) + "$"
        return self.assertRaisesRegexp(TempvarbSyntaxError, pat)

    def test_passthrough(self):
        # Strings without variables are passed through unchanged.
        self.assertEqual(Tempverb("Hello").render(), "Hello")
        self.assertEqual(
            Tempverb("Hello, 20% fun time!").render(),
            "Hello, 20% fun time!"
            )
    ...
    def test_reusability(self):
        # A single Tempverb can be used more than once with different data.
        globs = {
            'upper': lambda x: x.upper(),
            'punct': '!',
            }

        template = Tempverb("This is {{name|upper}}{{punct}}", globs)
