window.app = {
	 ...
	 * netty服务后端发布的url地址
	 */
	nettyServerUrl: 'ws://192.168.137.1:8000/ws',
	
	/**
	 * 后端服务发布的url地址
	 */
	// serverUrl: 'http://euraxluo.nat300.top',
	serverUrl: 'http://192.168.137.1:8888',
	
	/**
	 * 图片服务器的url地址
	 */
	imgServerUrl: 'http://47.107.44.224:8081/wss/',
	...
	setUserGlobalInfo: function(user) {
		var userInfoStr = JSON.stringify(user);//将其转为json
		plus.storage.setItem("userInfo", userInfoStr);//使用storage缓存
	},
	...
	getUserGlobalInfo: function() {
		var userInfoStr = plus.storage.getItem("userInfo");//从缓存中获取
	...
	getUserChatHistory: function(myId, friendId) {
		...
		var chatHistoryListStr = plus.storage.getItem(chatKey);
		var chatHistoryList;
		// console.log(JSON.stringify(chatHistoryListStr));
		if (me.isNotNull(chatHistoryListStr)) {
			// 如果不为空
			chatHistoryList = JSON.parse(chatHistoryListStr);
		} else {
